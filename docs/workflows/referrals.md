...
> **From a bundle service offer:** After demand assigns an accepted multi-service bundle, you receive **one referral per service** (separate `sub_inbox_referrals` events). Process each referral with the steps below. Sibling referrals share `summary.alayamarket_shift_id`. See [Bundle Service Offers](bundle-service-offers.md).
